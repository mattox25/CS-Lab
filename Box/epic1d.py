...
from numpy import arange, concatenate, zeros, linspace, floor, array, pi, zeros_like, where, stack, add
from numpy import exp, sin, cos, sqrt, random, histogram, abs, sqrt, max, savetxt, column_stack
import os
...
def calc_density_new(position, ncells, L):
    """ Calculate charge density given particle positions
    ...
    Output
      density   - contains 1 if evenly distributed
    """
    # Create an empty density array
    density = zeros(ncells)

    # Calculate dx (uniform cell spacing)
    dx = L / ncells

    # Normalize positions to cell indices
    normalized_positions = position / dx

    # Compute lower cell indices and fractional offsets
    cell_indices = floor(normalized_positions).astype(int)
    offsets = normalized_positions - cell_indices

    # Efficiently calculate the density contributions
    add.at(density, cell_indices, 1 - offsets)
    add.at(density, (cell_indices + 1) % ncells, offsets)

    # Normalize density so that the average density is 1
...
def pic(f, ncells, L):
    ...
    density = calc_density_new(pos, ncells, L)
...
# Define the exponential function model: y = a * exp(b * x)
